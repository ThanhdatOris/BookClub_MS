...
class ProfileController extends AbstractController
{
    ...
    #[Route('/admin/suggestions/{index}/approve', name: 'admin_suggestions_approve', methods: ['POST'])]
    public function approveSuggestion(int $index, Request $request): Response
    {
        ...
        if (!$this->isCsrfTokenValid('approve_suggestion' . $index, $request->request->get('_token'))) {
    ...
    #[Route('/admin/suggestions/{index}/reject', name: 'admin_suggestions_reject', methods: ['POST'])]
    public function rejectSuggestion(int $index, Request $request): Response
    {
        if (!$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('Chỉ admin mới có quyền thực hiện hành động này.');
        }

        if (!$this->isCsrfTokenValid('reject_suggestion' . $index, $request->request->get('_token'))) {
            $this->addFlash('error', 'CSRF token không hợp lệ.');
            return $this->redirectToRoute('admin_suggestions');
        }
